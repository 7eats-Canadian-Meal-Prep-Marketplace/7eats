...
// itself. Calendar days use the runtime-local convention the rest of the app
// uses for fulfillment windows (see cook-card-schedule.ts).
...
/**
 * Earliest moment a cook is allowed to mark the order ready: local midnight of
 * the calendar day before the scheduled fulfillment. Null when the order has no
 * schedule to guard against.
 */
